function [out] = lyapFilter2(varargin)
A = varargin{1};
B = varargin{2};
beta = varargin{3};
...
if nargin == 4
    lambda = varargin{4};
elseif nargin > 4
    error('Error. Maximum inputs exceed (4)');
end
% Filter's parameter
alfa = 1 - beta;
...
for i=1:N %varphik
    for j=1:N %varphik+1
        for m=1:N %sigmak
            matrix = [-G{i}-G{i}' A{m}*G{i}'+B{m}*K{i} G{i};G{i}*A{m}'+K{i}'*B{m}' (1/lambda)*P{i}-G{i}-G{i}' zeros(n,n);G{i}' zeros(n,n) -P{j}];
